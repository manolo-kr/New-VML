...
# 환경설정
# -------------------------------------------------------------------
API_DIR = os.getenv("API_DIR", "http://127.0.0.1:8065")   # 실제 백엔드 서버 주소
...
def upload_file_from_contents(contents: str, filename: str) -> Dict[str, Any]:
    """
    dcc.Upload → /upload 멀티파트 업로드
    반환: {"dataset_uri": "file://...", "original_name": "<원본이름>"}
    """
    if not contents:
...
def get_artifact_png_src(run_id: str, name: str) -> Optional[str]:
    try:
        b = get_artifact_file(run_id, name)
...
# Util
# -------------------------------------------------------------------
def list_models(run_id: str) -> List[str]:
    run = get_run(run_id)
    ...
    for k in candidates:
        try:
            _ = get_artifact_json(run_id, f"models/{k}/curves/roc.json")
            present.append(k)
        except Exception:
            pass
    return present