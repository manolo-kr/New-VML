...
# NOTE: 페이지 모듈 임포트는 "앱 인스턴스 생성 이후"에 이루어지도록
# build_dash_app() 내부에서 수행합니다. (register_page 순서 보장)
...
def _navbar():
    # NavbarText는 컴포넌트가 아니라서 Badge/Span으로 구성
...
def build_dash_app() -> dash.Dash:
    ...
    app.layout = dbc.Container([
        ...
        # 두 개의 Location을 분리: 가드와 로그인 성공 리다이렉트 충돌 방지
        dcc.Location(id="_guard_redirect", refresh=True),
        dcc.Location(id="_auth_redirect_sink"),  # login.py에서만 사용
    ...
    #    ※ _guard_redirect.href 는 이 콜백 "한 곳"에서만 출력 (중복 방지)
    # ─────────────────────────────────────────────
    @callback(
    ...
    # ─────────────────────────────────────────────
    # 3) 로그아웃 버튼 → gs-auth 초기화 & 로그인으로
    #    로그인/가드 리다이렉트와 출력 대상 분리됨(여기선 gs-auth만)
    # ─────────────────────────────────────────────
    @callback(
        Output("gs-auth", "data"),
    ...
    def _logout(_n):
        # 프런트 토큰만 비우면 됨(백엔드 /auth/logout 은 no-op)
        return None
